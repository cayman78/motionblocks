# Работа с устройством и данными — MotionBlocks

Этот документ описывает полный цикл работы с системой: от первой прошивки до обучения классификатора.

Перед использованием убедись что окружение настроено согласно [01_setup.md](01_setup.md).
...
M5StickC Plus2 — компактное устройство на базе ESP32 с IMU сенсором, экраном и тремя кнопками.

Если держать устройство так, чтобы экран смотрел на тебя, а фронтальная кнопка оказалась справа:

```text
Кнопка A        — фронтальная (справа от экрана) — основная управляющая
Кнопка B        — сверху устройства — вспомогательная
Кнопка питания  — снизу устройства
```

| Действие | Результат |
|---|---|
| Двойной клик кнопки A | Начать запись |
| Одиночный клик кнопки A во время записи | Остановить запись |
| Кнопка B | Следующая сессия |
| Удержание кнопки B | Выключить устройство |

---

## 1. Подготовка перед записью

### 1.1 Узнай IP-адрес компьютера
...
| `--device-id` | Идентификатор устройства, например `m5-01` |
...
Для большинства задач используй **100 Hz**.
...
Когда устройство появится на экране READY, logger выведет:
...
Logger в консоли будет печатать прогресс и итог каждой записи:
...
**Важно:** не использовать реальные имена детей — только псевдонимы (`child_01`, `child_02`).
...
## 6. Извлечение фич для ML

После того как данные проверены и метаданные заполнены (`movement_type` в motion_browser.py), можно извлечь фичи для обучения модели.

### 6.1 Убедись что movement_type заполнен

Скрипт пропускает сессии со значением `unknown`.

### 6.2 Запусти compute_features.py
...
## 7. Обучение классификатора в Orange Data Mining
...
### 7.3 Собрать pipeline для обучения
...
### 7.5 Предсказание на новых данных
